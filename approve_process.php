<?php
	...
	if(isset($_POST['is_approve']) && isset($_POST['id'])){
		...
		if($is_approve == 1){
			...
			$conn->exec($sql_stmt);
			$sql_stmt = "UPDATE notification SET status=1 WHERE from_code=2 AND from_item_id='$id'";
			$conn->exec($sql_stmt);
		}else if($is_approve == -1){
			$sql_stmt = "DELETE FROM user WHERE id=$id";
			$conn->exec($sql_stmt);
			$sql_stmt = "DELETE FROM notification WHERE from_code=2 AND from_item_id='$id'";
			$conn->exec($sql_stmt);
		}
	}else{
